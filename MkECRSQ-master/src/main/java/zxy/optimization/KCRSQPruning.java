...
public class KCRSQPruning<T> {

	public Long nodeNum = Long.parseUnsignedLong("0");
	public Long gans = Long.parseUnsignedLong("0");

	//根据预处理的Pre_dsg，执行kcrsq算法
	public ArrayList<ArrayList<GraphPoints<T>>> pWisePlus(ArrayList<ArrayList<GraphPoints<T>>> dsgs,int k){
		...
		for(int i=1;i<k;i++) {
			...
			for(ArrayList<GraphPoints<T>> combinations:k_skylineGcombinations.get(i-1)) {
				...
				for(int j=tail;j<pre_points.size();j++) {
					...
					if( cur_point.layer_index - combinations.get(combinations.size()-1).layer_index >= 2 ) break;
		...
		return k_skylineGcombinations.get(k_skylineGcombinations.size()-1);
	}
